...
def extract_test_metrics_from_json(json_file: Path) -> Dict[str, Any]:
    """
    Extrae las métricas de test de un archivo JSON de resultados LSTM.
    ...
    # LSTM usa "metrics_test" en lugar de "test_metrics"
    test_metrics = data.get("metrics_test", {})
    step = data.get("steps") or data.get(
        "step"
    )  # Intentar "steps" primero, luego "step" como fallback
    # LSTM usa "model" en lugar de "model_type"
    ...
    return {
        ...
        "stepwise_wmape": json.dumps(
            data.get("stepwise_wmape_test", {})
        ),  # LSTM tiene stepwise_wmape_test en el nivel raíz
    }


def extract_validation_metrics_from_json(json_file: Path) -> Dict[str, Any]:
    """
    Extrae las métricas de validación de un archivo JSON de resultados LSTM.
    ...
    # LSTM usa "metrics_validation" en lugar de "validation_metrics"
    validation_metrics = data.get("metrics_validation", {})
    step = data.get("steps") or data.get(
        "step"
    )  # Intentar "steps" primero, luego "step" como fallback
    # LSTM usa "model" en lugar de "model_type"
    ...
    return {
        ...
        "stepwise_wmape": json.dumps(
            data.get("stepwise_wmape_validation", {})
        ),  # LSTM tiene stepwise_wmape_validation en el nivel raíz
    }
...
def main():
    ...
    base_dir = Path("data/analytics/model_results/GIR-EPM/con_exog/lstm")
