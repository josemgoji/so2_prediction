"""
Script para consolidar métricas de test y validación de todos los steps y modelos baseline en CSV.

Recorre cada step en data/analytics/model_results/CEN-TRAF/con_exog/baseline/
...
def extract_test_metrics_from_json(json_file: Path) -> Dict[str, Any]:
    """
    Extrae las métricas de test de un archivo JSON de resultados baseline.
    ...
    step = data.get("step") or data.get(
        "steps"
    )  # Intentar "step" primero (baseline usa "step"), luego "steps" como fallback
...
def extract_validation_metrics_from_json(json_file: Path) -> Dict[str, Any]:
    """
    Extrae las métricas de validación de un archivo JSON de resultados baseline.
...
def main():
    """Función principal"""
    print("=" * 80)
    print("CONSOLIDACIÓN DE MÉTRICAS DE TEST Y VALIDACIÓN - BASELINE")
    print("=" * 80)

    base_dir = Path("data/analytics/model_results/MED-FISC/con_exog/baseline")
    ...
    if not df_test.empty:
        output_file_test = base_dir / "test_metrics_all_steps_baseline.csv"
    ...
    if not df_validation.empty:
        output_file_validation = base_dir / "validation_metrics_all_steps_baseline.csv"
    ...
    if not df_test.empty:
        print(f"✅ CSV de TEST: {base_dir / 'test_metrics_all_steps_baseline.csv'}")
    if not df_validation.empty:
        print(
            f"✅ CSV de VALIDACIÓN: {base_dir / 'validation_metrics_all_steps_baseline.csv'}"
        )
