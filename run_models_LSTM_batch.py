"""
Script para ejecutar entrenamiento de modelos LSTM para múltiples estaciones y steps.

Este script permite ejecutar el entrenamiento de modelos LSTM en batch,
procesando múltiples combinaciones de estaciones y steps de forma secuencial.
"""
...
from models_LSTM import train_and_evaluate_models
...
def run_batch_training(
    ...
    """
    Ejecuta el entrenamiento de modelos LSTM para múltiples estaciones y steps.
    ...
    steps : list[int]
        Lista de steps de predicción en horas (ej: [6, 24, 72]) - pasos hacia adelante que se predicen
    use_exog : bool
    ...
    horizon : int
        Horizonte para shift del target (0 = no shift, >0 = shift hacia adelante).
        Si horizon > 0, el target se shifteará para predecir desde el paso 'horizon'.
    param_grid : dict, optional
        Grid de hiperparámetros para búsqueda. Si None, usa valores por defecto en models_LSTM.py.
    ...
    print(f"\n{'=' * 80}")
    print("INICIANDO ENTRENAMIENTO EN BATCH (LSTM)")
    print(f"{'=' * 80}")
    ...
    for station in stations:
        for step in steps:
            ...
            print(f"Estación: {station}, Step: {step}")
            if horizon > 0:
                print(f"Horizonte de shift: {horizon}")
            print(f"{'#' * 80}\n")
            ...
            try:
                ...
                print(f"\n✅ Completado: {station} - S{step}")
                if horizon > 0:
                    print(f"   Horizonte: {horizon}")

            except Exception as e:
                print(f"\n❌ ERROR en {station} - S{step}: {str(e)}")
                import traceback
    ...
    batch_summary_dir = Path(
        f"data/analytics/model_results/batch_summaries_lstm_{timestamp_str}"
    )
    ...
    for station in stations:
        ...
        for step in steps:
            ...
            if key in all_batch_results and all_batch_results[key].get(
                ...
                for model_result in result.get("results", []):
                    ...
                    station_models[model_name].append({"step": step, **model_result})
        ...
        for model_name, model_results in station_models.items():
            # Crear DataFrame con todos los steps de este modelo
            rows = []

            for model_result in sorted(model_results, key=lambda x: x["step"]):
                row = {
                    "step": model_result["step"],
            ...
            print(f"🔢 Steps incluidos: {sorted([r['step'] for r in model_results])}")
            if horizon > 0:
                print(f"   Horizonte de shift: {horizon}")
    ...
    print(f"\n{'=' * 80}")
    print("RESUMEN FINAL DEL BATCH (LSTM)")
    print(f"{'=' * 80}")
...
if __name__ == "__main__":
    ...
    # Grid de hiperparámetros (opcional, si None usa valores por defecto)
    PARAM_GRID = None  # O personaliza con:
    # PARAM_GRID = {
