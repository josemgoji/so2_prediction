# --- baseline_batch_persistence.py --------------------------------------------
"""
Ejecución en batch del baseline de persistencia (last value) con skforecast
para múltiples estaciones y steps.
"""
...
# Importa tu función del nuevo baseline (persistencia skforecast)
from base_lineV2 import train_and_evaluate_models_persistence


def run_batch_training_persistence(
    stations: List[str],
    steps: List[int],
    use_exog: bool = True,  # compat (no se usan exógenas en este baseline)
    ...
    print(f"\n{'=' * 80}")
    print("INICIANDO ENTRENAMIENTO EN BATCH (Baseline Persistencia - skforecast)")
    print(f"{'=' * 80}")
    ...
    print(
        f"Configuración: {'Con exógenas' if use_exog else 'Sin exógenas'} "
        "(baseline ignora exógenas)"
    )
    print(f"{'=' * 80}\n")
    ...
    for station in stations:
        for step in steps:
            ...
            try:
                result = train_and_evaluate_models_persistence(
                    station=station,
                    step=step,
                    use_exog=use_exog,
                    n_trials=n_trials,
    ...
    summary_file = batch_summary_dir / "batch_summary.json"
...
if __name__ == "__main__":
    ...
    TEST_MONTHS = 2
    HORIZON = 0
    BATCH_NAME = "persistence_skforecast"

    _ = run_batch_training_persistence(
        stations=STATIONS,
        steps=STEPS,
        use_exog=USE_EXOG,
        n_trials=N_TRIALS,
